...
docker build . -f clickhouse/Dockerfile-clickhouse-build --build-arg BASE=test-arch-no-lto-basic -t ch-test-arch

docker build . -f clickhouse/Dockerfile-clickhouse-build --build-arg BASE=test-alhp-v4-no-lto-native --build-arg CH_ARCH=native -t ch-test-alhp-v4-native
...
for f in php/Dockerfile.*; do
    ...
    if [ "$a" != "" ]; then
        if [ "$f" != 'Dockerfile.alhp-v4']; then
            docker build . -f clickhouse/Dockerfile-clickhouse-copy --build-arg BASE=test-$type-no-lto-native --build-arg COPY_FROM=ch-test-alhp-v4-native -t ch-test-$type-native
        ...
        docker build . -f clickhouse/Dockerfile-clickhouse-build --build-arg BASE=test-$type-no-lto-native --build-arg CH_ARCH=$a -t ch-test-$type-$a
    ...
    docker build . -f clickhouse/Dockerfile-clickhouse --build-arg BASE=test-$type-no-lto-basic -t ch-test-$type
