...
for f in php/Dockerfile.*; do
    ...
    if [ "$a" != "" ]; then
        if [ "$f" != 'Dockerfile.alhp-v4' ]; then
            ...
            cmds="docker run --rm -v $PWD/test-results:/var/lib/phoronix-test-suite/test-results/ -it ch-test-$type-native sh -c 'clickhouse-server  --daemon && /run.sh ch-test-$type-native clickhouse'; $cmds"
        fi
    fi
