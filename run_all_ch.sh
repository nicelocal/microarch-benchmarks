...
for f in Dockerfile.*; do
    ...
    docker build . --build-arg BASE=test-$type-no-lto -t ch-test-$type
    ...
    if grep ARCH $f; then
        docker build . --build-arg BASE=test-$type-no-lto-native --build-arg ARCH=native -t ch-test-$type-native
