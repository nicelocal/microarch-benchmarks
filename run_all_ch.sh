...
for f in php/Dockerfile.*; do
    ...
    cmds="docker run --rm -v $PWD/test-results:/var/lib/phoronix-test-suite/test-results/ -it ch-test-$type sh -xec 'clickhouse-server --daemon && /run.sh ch-test-$type clickhouse'; $cmds"
done
